...
export class PayoutService {
    DAILY_PAYOUT_AMOUNT = 500.0; // Rs. 500 daily income replacement
    /**
    ...
    async executeEventPayouts(eventId, city) {
        try {
                ...
                try {
                    ...
                    await prisma.$transaction(async (tx) => {
                        ...
                        const updatedWallet = await tx.wallet.update({
                            ...
                            data: {
                                balance: {
                                    increment: this.DAILY_PAYOUT_AMOUNT,
                                },
                            },
                        });
                        console.log(`[PAYOUT] Wallet updated for User ${policy.userId}: +${this.DAILY_PAYOUT_AMOUNT}, New Balance: ${updatedWallet.balance}`);
                        // B. Record the payout ledger entry (IMMUTABLE)
                        const payoutRecord = await tx.payout.create({
                            data: {
                                userId: policy.userId,
                                eventId,
                                amount: this.DAILY_PAYOUT_AMOUNT,
                                status: 'success',
                            },
                        });
                        console.log(`[PAYOUT SUCCESS] Payout recorded: User ${policy.userId} received ₹${this.DAILY_PAYOUT_AMOUNT}. Payout ID: ${payoutRecord.id}`);
                    });
                    successCount++;
                }
                catch (txError) {
            ...
            console.log(`[PAYOUT SUMMARY] Event: ${eventId}, City: ${city}, Success: ${successCount}/${affectedPolicies.length}, Failed: ${failureCount}`);
        }
        catch (error) {
     ...
     */
    async getEventPayoutStats(eventId) {

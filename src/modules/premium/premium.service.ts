    ...
    try {
      ...
      for (const user of users) {
        ...
        try {
          ...
          console.log(
            `[PREMIUM] User ${user.id}: Base=${this.BASE_PREMIUM}, Risk Multiplier=${riskMultiplier.toFixed(2)}, Loyalty Discount=${(this.LOYALTY_DISCOUNT * 100).toFixed(0)}%, Final=₹${finalPremium.toFixed(2)}`
          );
          ...
          await prisma.$transaction(async (tx) => {
            ...
            const updatedWallet = await tx.wallet.update({
              ...
              data: {
                balance: {
                  decrement: finalPremium as unknown as any,
                },
              },
            });

            console.log(
              `[PREMIUM] Wallet charged: User ${user.id}, -₹${finalPremium.toFixed(2)}, Remaining: ₹${updatedWallet.balance}`
            );
            ...
            console.log(
              `[PREMIUM SUCCESS] Policy created for User ${user.id}: ${nextSunday.toDateString()} - ${nextSaturday.toDateString()}, Policy ID: ${policy.id}`
            );
          });
      ...
      console.log(
        `[PREMIUM SUMMARY] Weekly Renewals Complete - Success: ${successCount}, Failed: ${failureCount}, Skipped: ${skipCount}`
      );
    } catch (error) {
